...
class IpRepository(
    ...
    fetcher = DataFetcher.of(
        fetch = { request ->
            Log.d("BhandarExample", "$request: Fetching from network...")
            try {
    ...
    storage = Storage.of(
        read = { request ->
            Log.d("BhandarExample", "$request: Reading from local...")
            memoryStore
        },
        isValid = { it?.isNotBlank() == true },
        write = { request, newValue ->
            Log.d("BhandarExample", "$request: Writing $newValue to local...")
